classdef EnergiSenseInteractiveDashboard < matlab.apps.AppBase
    ...
    properties (Access = public)
        ...
        VerificationButton              matlab.ui.control.Button
    ...
    methods (Access = private)
        ...
        function createEnhancedButtons(app)
            ...
            % Create VerificationButton
            app.VerificationButton = uibutton(app.ControlsGrid, 'push');
            app.VerificationButton.ButtonPushedFcn = createCallbackFcn(app, @launchVerificationSystem, true);
            app.VerificationButton.BackgroundColor = [0.6 0.2 0.8]; % Professional purple
            app.VerificationButton.FontColor = [1 1 1];
            app.VerificationButton.FontWeight = 'bold';
            app.VerificationButton.FontSize = 12;
            app.VerificationButton.Text = '🔍 Verification';
            app.VerificationButton.Icon = '';
            app.VerificationButton.Layout.Row = 8;
            app.VerificationButton.Layout.Column = 1;
        end
    ...
    methods (Access = private)
        ...
        function [predictedPower, confidence] = enhancedPredictPower(app, params)
            ...
            % FIXED: Correct input order for ML models [AT, V, AP, RH] = [Temperature, Vacuum, Pressure, Humidity]
            inputVector = [params.temperature, params.vacuum, params.pressure, params.humidity];
            confidence = 0.95; % Default confidence
            
            try
                % Debug info for troubleshooting
                if app.DebugMode
                    fprintf('Debug: ModelType = %s, Input = [%.1f, %.1f, %.1f, %.1f]\n', ...
                        app.ModelType, inputVector);
                end
                ...
                switch app.ModelType
                    case "enhanced_function"
                        ...
                        try
                            ...
                            if ~exist('predictPowerEnhanced', 'file')
                                fprintf('⚠️ predictPowerEnhanced not found in path, adding core/prediction\n');
                                addpath('core/prediction');
                                addpath('../core/prediction');
                                addpath('../../core/prediction');
                            end
                            
                            [predictedPower, pred_confidence] = predictPowerEnhanced(inputVector);
                            confidence = pred_confidence;
                            if app.DebugMode
                                fprintf('Debug: Enhanced function prediction successful: %.2f MW\n', predictedPower);
                            end
                        catch ME
                            fprintf('⚠️ Enhanced function failed: %s, falling back to empirical\n', ME.message);
                            if app.DebugMode
                                fprintf('Debug: Error details: %s\n', ME.message);
                                if ~isempty(ME.stack)
                                    fprintf('Debug: Error location: %s line %d\n', ME.stack(1).name, ME.stack(1).line);
                                end
                            end
                    ...
                    case {"reconstructed_ensemble", "reconstructed_from_struct"}
                        ...
                        try
                            if app.DebugMode
                                fprintf('Debug: Using ensemble model, type: %s\n', class(app.EnsembleModel));
                            end
                            
                            predictedPower = predict(app.EnsembleModel, inputVector);
                            confidence = 0.99;
                            if app.DebugMode
                                fprintf('Debug: Ensemble model prediction successful: %.2f MW\n', predictedPower);
                            end
                        catch ME
                            fprintf('⚠️ Ensemble model failed: %s, falling back to function\n', ME.message);
                            if app.DebugMode
                                fprintf('Debug: Ensemble error details: %s\n', ME.message);
                            end
                            
                            % Try enhanced function as backup
                            try
                                [predictedPower, pred_confidence] = predictPowerEnhanced(inputVector);
                                confidence = pred_confidence;
                                fprintf('✅ Fallback to enhanced function successful\n');
                            catch ME2
                                fprintf('⚠️ Enhanced function also failed: %s, using empirical\n', ME2.message);
                                predictedPower = app.enhancedEmpiricalPrediction(inputVector);
                                confidence = 0.75;
                            end
                        end
                    ...
                    otherwise
                        % Final fallback
                        fprintf('⚠️ Unknown model type: %s, using empirical fallback\n', app.ModelType);
                        predictedPower = app.enhancedEmpiricalPrediction(inputVector);
                        confidence = 0.80;
                end
        ...
        function power = enhancedEmpiricalPrediction(app, input)
            ...
            if app.ModelLoaded && isstruct(app.EnsembleModel) && isfield(app.EnsembleModel, 'coefficients')
                ...
                % FIXED: Proper coefficient order [AT, V, AP, RH, intercept] to match input vector
                AT = input(1); % Temperature
                V = input(2);  % Vacuum
                AP = input(3); % Pressure
                RH = input(4); % Humidity
                
                power = coeffs(5) + coeffs(1)*AT + coeffs(2)*V + coeffs(3)*AP + coeffs(4)*RH;
                ...
                % Enhanced empirical model based on CCPP thermodynamics
                base_power = 454.365;
        ...
        function launchSimpleVerification(app, ~)
            % Launch simple verification system (embedded)
            ...
            try
                ...
                try
                    ...
                    if power > 400 && power < 500
                        ...
                        mlStatus = '❌ Invalid range';
                    end
                catch ME
                    fprintf('   ❌ ML Prediction: ERROR - %s\n', ME.message);
                    mlStatus = sprintf('❌ ERROR - %s', ME.message);
                end
                ...
                try
                    ...
                    weatherStatus = '✅ WORKING';
                catch ME
                    fprintf('   ❌ Weather System: ERROR - %s\n', ME.message);
                    weatherStatus = sprintf('❌ ERROR - %s', ME.message);
                end
                ...
                fileStatus = sprintf('✅ %d/%d files found', filesFound, length(modelFiles));
                
                % Summary
                fprintf('\n=== VERIFICATION SUMMARY ===\n');
                fprintf('ML Prediction: %s\n', mlStatus);
                fprintf('Weather System: %s\n', weatherStatus);
                fprintf('Model Files: %s\n', fileStatus);
                fprintf('\nVerification complete!\n\n');
                
                % Show completion dialog
                uialert(app.UIFigure, ...
                    sprintf('Verification complete!\n\nML Prediction: %s\nWeather: %s\nFiles: %s\n\nCheck command window for details.', ...
                    strrep(mlStatus, '❌', 'ERROR'), strrep(weatherStatus, '❌', 'ERROR'), strrep(fileStatus, '❌', 'MISSING')), ...
                    'Verification Results', 'Icon', 'info');
                
                fprintf('✅ Verification system completed!\n');
                
            catch ME
                fprintf('❌ Verification failed: %s\n', ME.message);
                uialert(app.UIFigure, ...
                    sprintf('Verification failed: %s', ME.message), ...
                    'Verification Error', 'Icon', 'error');
            end
        end

        function launchVerificationSystem(app, ~)
            % Launch comprehensive verification system (professional GUI)
            
            try
                fprintf('🔍 Launching Professional Verification System...\n');
                
                % Try to launch the compatible professional GUI first
                try
                    compatibleVerificationGUI();
                    fprintf('✅ Compatible verification GUI launched\n');
                    return;
                catch ME
                    fprintf('⚠️ Compatible GUI unavailable: %s\n', ME.message);
                    % Try original professional GUI
                    try
                        professionalVerificationGUI();
                        fprintf('✅ Original professional GUI launched\n');
                        return;
                    catch ME2
                        fprintf('⚠️ Professional GUI unavailable: %s\n', ME2.message);
                        fprintf('📋 Falling back to integrated verification dialog\n');
                    end
                end
                
                % Create verification dialog
                verificationFig = uifigure('Name', 'EnergiSense Verification System', ...
                    'Position', [200 200 800 600], 'Color', [0.1 0.1 0.1]);
                
                % Create main grid layout
                mainGrid = uigridlayout(verificationFig, [6 2]);
                try
                    mainGrid.RowHeights = {'50px', '1x', '50px', '100px', '50px', '50px'};
                    mainGrid.ColumnWidths = {'1x', '1x'};
                catch
                    % Fallback for older MATLAB versions
                    fprintf('Using compatible grid layout\n');
                end
                
                % Title
                titleLabel = uilabel(mainGrid, 'Text', '🔍 System Verification & Testing', ...
                ...
                resultsPanel = uipanel(mainGrid, 'Title', 'Verification Results', ...
                    'TitlePosition', 'centertop', 'BackgroundColor', [0.15 0.15 0.15], ...
                ...
                % Results text area
                try
                    resultsArea = uitextarea(resultsPanel, 'Position', [10 10 760 300], ...
                        'BackgroundColor', [0.05 0.05 0.05], 'FontColor', 'white', ...
                        'FontName', 'Courier New', 'FontSize', 11, 'Editable', 'off');
                catch
                    % Fallback to listbox for older versions
                    resultsArea = uilistbox(resultsPanel, 'Position', [10 10 760 300], ...
                        'BackgroundColor', [0.05 0.05 0.05], 'FontColor', 'white', ...
                        'FontName', 'Courier New', 'FontSize', 11);
                end
                
                % Test buttons panel
                buttonPanel = uipanel(mainGrid, 'Title', 'Quick Tests', ...
                    'BackgroundColor', [0.15 0.15 0.15], 'ForegroundColor', 'white');
                buttonPanel.Layout.Row = 3;
                buttonPanel.Layout.Column = [1 2];
                
                % Test buttons
                testGrid = uigridlayout(buttonPanel, [1 4]);
                
                mlTestBtn = uibutton(testGrid, 'Text', 'Test ML Model', ...
                    'BackgroundColor', [0.2 0.6 0.2], 'FontColor', 'white');
                mlTestBtn.ButtonPushedFcn = @(~,~) runMLTest(resultsArea);
                
                weatherTestBtn = uibutton(testGrid, 'Text', 'Test Weather', ...
                    'BackgroundColor', [0.2 0.4 0.8], 'FontColor', 'white');
                weatherTestBtn.ButtonPushedFcn = @(~,~) runWeatherTest(resultsArea);
                
                systemTestBtn = uibutton(testGrid, 'Text', 'System Check', ...
                    'BackgroundColor', [0.8 0.4 0.2], 'FontColor', 'white');
                systemTestBtn.ButtonPushedFcn = @(~,~) runSystemCheck(resultsArea);
                
                fullTestBtn = uibutton(testGrid, 'Text', 'Full Verification', ...
                    'BackgroundColor', [0.6 0.2 0.8], 'FontColor', 'white');
                fullTestBtn.ButtonPushedFcn = @(~,~) runFullVerification(resultsArea);
                
                % Status panel
                statusPanel = uipanel(mainGrid, 'Title', 'System Status', ...
                    'BackgroundColor', [0.15 0.15 0.15], 'ForegroundColor', 'white');
                ...
                uilabel(statusGrid, 'Text', 'Dashboard Status:', 'FontColor', 'white');
                dashStatus = uilabel(statusGrid, 'Text', '✅ Running', 'FontColor', 'green');
                
                uilabel(statusGrid, 'Text', 'ML Model Status:', 'FontColor', 'white');
                if app.ModelLoaded
                    mlStatus = uilabel(statusGrid, 'Text', sprintf('✅ Loaded (%s)', app.ModelType), 'FontColor', 'green');
                else
                    mlStatus = uilabel(statusGrid, 'Text', '❌ Not Loaded', 'FontColor', 'red');
                end
                
                uilabel(statusGrid, 'Text', 'Current Accuracy:', 'FontColor', 'white');
                accuracyStatus = uilabel(statusGrid, 'Text', sprintf('%.1f%%', app.ModelAccuracy), 'FontColor', 'cyan');
                
                % Control buttons
                controlPanel = uipanel(mainGrid, 'BackgroundColor', [0.15 0.15 0.15]);
                controlPanel.Layout.Row = 5;
                controlPanel.Layout.Column = [1 2];
                
                controlGrid = uigridlayout(controlPanel, [1 3]);
                
                exportBtn = uibutton(controlGrid, 'Text', 'Export Report', ...
                    'BackgroundColor', [0.4 0.4 0.4], 'FontColor', 'white');
                
                clearBtn = uibutton(controlGrid, 'Text', 'Clear Results', ...
                    'BackgroundColor', [0.6 0.3 0.3], 'FontColor', 'white');
                clearBtn.ButtonPushedFcn = @(~,~) clearResults(resultsArea);
                
                closeBtn = uibutton(controlGrid, 'Text', 'Close', ...
                    'BackgroundColor', [0.5 0.5 0.5], 'FontColor', 'white');
                closeBtn.ButtonPushedFcn = @(~,~) delete(verificationFig);
                
                % Auto-run initial verification
                runInitialVerification(resultsArea, app);
                
                fprintf('✅ Verification system launched successfully!\n');
                
            catch ME
                fprintf('❌ Verification system launch failed: %s\n', ME.message);
                uialert(app.UIFigure, sprintf('Verification launch failed: %s', ME.message), ...
                    'Verification Error', 'Icon', 'error');
            end
        end
...
% Verification System Helper Functions
function runInitialVerification(resultsArea, app)
    appendResults(resultsArea, '=== EnergiSense System Verification ===');
    appendResults(resultsArea, sprintf('Timestamp: %s', datetime('now')));
    appendResults(resultsArea, '');
    
    % Dashboard status
    appendResults(resultsArea, '🖥️ Dashboard Status:');
    appendResults(resultsArea, sprintf('   ✅ Interactive Dashboard: Running'));
    appendResults(resultsArea, sprintf('   📊 Model Type: %s', app.ModelType));
    appendResults(resultsArea, sprintf('   🎯 Accuracy: %.1f%%', app.ModelAccuracy));
    appendResults(resultsArea, '');
    
    % Model verification
    appendResults(resultsArea, '🤖 ML Model Verification:');
    if app.ModelLoaded
        appendResults(resultsArea, '   ✅ Model Status: Loaded');
        appendResults(resultsArea, sprintf('   📈 Type: %s', app.ModelType));
        appendResults(resultsArea, sprintf('   🎯 Accuracy: %.1f%%', app.ModelAccuracy));
    else
        appendResults(resultsArea, '   ❌ Model Status: Not Loaded');
    end
    appendResults(resultsArea, '');
    
    appendResults(resultsArea, '✅ Initial verification complete');
    appendResults(resultsArea, 'Use buttons above for detailed testing');
end

function runSystemCheck(resultsArea)
    appendResults(resultsArea, '');
    appendResults(resultsArea, '🔧 Running System Health Check...');
    
    try
        % Check core functions
        appendResults(resultsArea, '   Checking core functions:');
        ...
        for i = 1:length(functions_to_check)
            ...
            if exist(func_name, 'file')
                appendResults(resultsArea, sprintf('   ✅ %s: Available', func_name));
            else
                appendResults(resultsArea, sprintf('   ❌ %s: Missing', func_name));
            end
        end
        
        % Check model files
        appendResults(resultsArea, '');
        appendResults(resultsArea, '   Checking model files:');
        ...
        for i = 1:length(model_files)
            file_name = model_files{i};
            if exist(file_name, 'file')
                appendResults(resultsArea, sprintf('   ✅ %s: Found', file_name));
            else
                appendResults(resultsArea, sprintf('   ❌ %s: Missing', file_name));
            end
        end
        
        appendResults(resultsArea, '');
        appendResults(resultsArea, '✅ System Health Check: COMPLETED');
        
    catch ME
        appendResults(resultsArea, sprintf('   ❌ System Check Failed: %s', ME.message));
        appendResults(resultsArea, '❌ System Health Check: FAILED');
    end
end

function runFullVerification(resultsArea)
    appendResults(resultsArea, '');
    appendResults(resultsArea, '🚀 Running Full System Verification...');
    appendResults(resultsArea, '');
    
    % Run all tests
    runMLTest(resultsArea);
    runWeatherTest(resultsArea);
    runSystemCheck(resultsArea);
    
    appendResults(resultsArea, '');
    appendResults(resultsArea, '=== FULL VERIFICATION COMPLETE ===');
    appendResults(resultsArea, sprintf('Completed at: %s', datetime('now')));
end

function runMLTest(resultsArea)
    appendResults(resultsArea, '');
    appendResults(resultsArea, '🧪 Running ML Model Test...');
    ...
    try
        ...
        % Add debugging info
        appendResults(resultsArea, sprintf('   Input: [AT=%.0f°C, V=%.0f cmHg, AP=%.0f mbar, RH=%.0f%%]', testInput));
        ...
        appendResults(resultsArea, sprintf('   ✅ Predicted Power: %.2f MW', power));
        appendResults(resultsArea, sprintf('   ✅ Confidence: %.1f%%', confidence*100));
        appendResults(resultsArea, sprintf('   ⏱️ Prediction Time: %.3f seconds', elapsed));
        
        % Validate result range
        if power >= 400 && power <= 500
            appendResults(resultsArea, '   ✅ Result within expected range (400-500 MW)');
        else
            appendResults(resultsArea, sprintf('   ⚠️ Result outside expected range: %.2f MW', power));
        end
        ...
        appendResults(resultsArea, '');
        appendResults(resultsArea, '   🔄 Multiple Scenario Test:');
        ...
        for i = 1:size(testScenarios, 1)
            try
                ...
                if pow >= 400 && pow <= 500
                    appendResults(resultsArea, sprintf('   ✅ Scenario %d: %.1f MW (%.0f%% conf)', i, pow, conf*100));
                else
                    appendResults(resultsArea, sprintf('   ⚠️ Scenario %d: %.1f MW (outside range)', i, pow));
                    allScenariosPass = false;
                end
            catch ME
                appendResults(resultsArea, sprintf('   ❌ Scenario %d: ERROR - %s', i, ME.message));
                allScenariosPass = false;
            end
        end
        
        if allScenariosPass
            appendResults(resultsArea, '   ✅ All scenarios passed');
        end
        
        appendResults(resultsArea, '');
        appendResults(resultsArea, '✅ ML Model Test: PASSED');
        
    catch ME
        appendResults(resultsArea, sprintf('   ❌ ML Test Failed: %s', ME.message));
        appendResults(resultsArea, '   📋 Stack trace:');
        for i = 1:min(3, length(ME.stack))
            appendResults(resultsArea, sprintf('      %s (line %d)', ME.stack(i).name, ME.stack(i).line));
        end
        appendResults(resultsArea, '❌ ML Model Test: FAILED');
    end
end

function runWeatherTest(resultsArea)
    appendResults(resultsArea, '');
    appendResults(resultsArea, '🌤️ Running Weather Intelligence Test...');
    
    try
        weatherData = getWeatherIntelligence();
        
        appendResults(resultsArea, sprintf('   ✅ Temperature: %.1f°C', weatherData.temperature));
        appendResults(resultsArea, sprintf('   ✅ Humidity: %.1f%%', weatherData.humidity));
        appendResults(resultsArea, sprintf('   ✅ Pressure: %.1f mbar', weatherData.pressure));
        appendResults(resultsArea, sprintf('   ✅ Vacuum: %.1f cmHg', weatherData.vacuum));
        
        if isfield(weatherData, 'forecast')
            appendResults(resultsArea, sprintf('   ✅ Forecast: %s', weatherData.forecast.summary));
        end
        
        if isfield(weatherData, 'impact')
            appendResults(resultsArea, sprintf('   ✅ Efficiency Impact: %.1f%%', weatherData.impact.efficiencyLoss));
        end
        
        appendResults(resultsArea, '');
        appendResults(resultsArea, '✅ Weather Intelligence Test: PASSED');
        
    catch ME
        appendResults(resultsArea, sprintf('   ❌ Weather Test Failed: %s', ME.message));
        appendResults(resultsArea, '❌ Weather Intelligence Test: FAILED');
    end
end

function appendResults(resultsArea, text)
    try
        % Try textarea approach first
        current = get(resultsArea, 'Value');
        if ischar(current)
            current = {current};
        ...
        current{end+1} = text;
        set(resultsArea, 'Value', current);
    catch
        % Fallback for listbox
        try
            current = get(resultsArea, 'Items');
            if isempty(current)
                current = {};
            end
            current{end+1} = text;
            set(resultsArea, 'Items', current);
            % Auto-scroll to bottom
            if length(current) > 1
                set(resultsArea, 'Value', current{end});
            end
        catch ME
            % Final fallback - just display in command window
            fprintf('%s\n', text);
        end
    end
...
function clearResults(resultsArea)
    try
        % Try textarea approach
        set(resultsArea, 'Value', {});
    catch
        try
            % Try listbox approach
            set(resultsArea, 'Items', {});
        catch
