...
try
    % Use direct ML prediction to avoid circular dependency
    [prediction, confidence_struct, ~] = predictPowerML(inputData);
    if isstruct(confidence_struct) && isfield(confidence_struct, 'mean_confidence')
        confidence = mean(confidence_struct.mean_confidence);
    else
        confidence = 0.90;
    end
catch ME
    % Basic fallback using empirical model
